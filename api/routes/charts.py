...
async def _run_charts_job(repo_id: str, repo_url: str, provider: str, model_name: str, github_token: str = None):
    try:
...
async def start_charts(repo_id: str, background_tasks: BackgroundTasks, payload: ChartsRequest = ChartsRequest(), user: User | None = Depends(get_current_user)):
    ...
    # Pass the user's token (if any) so private-repo contributor/issue stats work.
    github_token = user.github_token if user else None
    background_tasks.add_task(_run_charts_job, repo_id, repo_url, payload.provider or "openrouter", payload.model_name, github_token)
...
async def get_charts(repo_id: str, user: User | None = Depends(get_current_user)):
    await require_repo_access(repo_id, user)
